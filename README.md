...
```powershell
cd D:\proj\image-similarity-sorter
...
```powershell
python -m glass_image_sorter.cli D:\photos --dry-run --output preview.csv
```

如果已经执行 `python -m pip install -e .`，也可以使用命令别名：

```powershell
lumaorder D:\photos --dry-run --output preview.csv
```

执行原地重命名：

```powershell
python -m glass_image_sorter.cli D:\photos --apply --output result.csv
```

生成抽查联系表：

```powershell
python -m glass_image_sorter.cli D:\photos --dry-run --contact-sheets review_samples
```
...
```json
{
  ...
  "output": "D:\\photos\\preview.csv",
  "contactSheets": "D:\\photos\\review_samples"
}
```
...
```text
D:\photobackup\review_samples
```

项目只复制了少量联系表到 `docs/examples`，用于 Web UI 示例列表和 README 预览，不复制完整样例集。

![DCIM start](docs/examples/D_photobackup_DCIM_final_start.jpg)

![DCIM middle](docs/examples/D_photobackup_DCIM_final_middle.jpg)

![Screenshots start](docs/examples/D_photobackup_DCIM_Screenshots_final_start.jpg)
